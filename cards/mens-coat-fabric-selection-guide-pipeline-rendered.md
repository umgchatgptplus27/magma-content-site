...
- 임시 개발 서버의 수명은 worker 세션이 아니라 worktree 안의 run별 preview lease가 관리한다. 카드 6·7은 `scripts/development-preview-supervisor.mjs ensure`로 같은 lease를 조회하고, 포트 LISTEN만이 아니라 `/`와 `/blog`의 순차 HTTP 200 readiness를 확인한다. lease가 없거나 죽었으면 이 helper만 동일 worktree·포트에서 재기동할 수 있으며, 이는 개발 POST를 다시 호출하지 않는다. 카드 7의 `needs_input` 운영 승인 대기까지 유지하고, 운영 발행 카드가 terminal `done` 또는 재개 전 별도 조정이 필요한 `capability` Blocked로 끝날 때만 helper의 `stop`으로 종료·포트 해제를 확인한다.
...
10. Google SEO Starter Guide의 기술 기본 요건을 개발 미리보기에서 검토한다. 다음 명령으로 글 상세·sitemap·robots를 읽기 전용으로 검사하고, 결과 JSON을 카드에 남긴다. 이 검사는 색인 또는 검색 순위를 보장하는 검사가 아니다.
   ...
   통과 기준은 HTTP 200, `lang="ko"`, title·description·H1, 대표 `www` canonical, OG·Twitter metadata, Article·Breadcrumb JSON-LD, 이미지 alt 속성, `sitemap.xml`의 공개 URL 포함 및 `robots.txt`의 대표 sitemap 선언이다.
11. 위 콘텐츠·이미지·기술 SEO 검증이 모두 통과한 뒤 카드를 완료한다. 이 성공만으로 임시 서버를 종료하지 않고, run별 preview lease와 로그 경로를 운영 발행 카드에 인계한다. 운영 발행 카드는 동일 URL에서 `ensure`로 health를 재확인하거나, 죽은 lease를 개발 POST 없이 안전하게 재기동한 뒤 운영 승인 대기를 진행한다.
12. preflight 오류, HTTP 오류, 409, 타임아웃, 연결 끊김, 파싱 실패 또는 불명확한 결과면 재시도하지 않고 비밀값을 제거해 보고한 뒤 수동 조정이 필요한 `capability` Blocked로 전환한다.
...
[합격 기준]
최종 원고 승인과 기존 전송 기록을 먼저 확인한다. `scripts/development-preview-supervisor.mjs ensure`가 run별 detached lease에서 `/`·`/blog` HTTP 200 readiness를 확인한 뒤, `scripts/development-publish-once.mjs`가 해당 slug 한 건의 승인 원고를 무쓰기 preflight하고 date를 `YYYY-MM-DD`로 정규화·달력 검증한 뒤에만 상태 원장을 기록하고 개발 API를 `draft: false`로 정확히 한 번 호출한다. 어떤 결과에서도 POST를 재시도하지 않는다. 상세 페이지가 HTTP 200이며 승인된 제목·본문·본문 이미지 5장을 표시하고, 블로그 목록이 해당 글 링크·승인된 썸네일 1장을 표시하며, 전체 이미지 파일 6장이 모두 HTTP 200인지 읽기 전용으로 검증한 뒤 두 번째 승인 Blocked 없이 완료한다. 이 카드의 성공은 서버 종료 신호가 아니며, worker가 끝나도 lease가 운영 발행 카드의 재검증과 운영 승인 대기까지 살아 있어야 한다. 오류나 불명확 결과는 비밀값 없이 보고하고 운영 쓰기 0회를 유지한다.
...
5. 대상·범위·단일 실행 조건을 대표에게 제시하고 운영 쓰기 없이 `needs_input` Blocked로 이동한다. 차단 사유는 `운영 대상·범위·단일 실행 승인 대기 — 운영 쓰기 0회`로 기록한다. 이 승인 대기 중에는 임시 서버를 종료하지 않고 개발 미리보기 URL을 유지한다. 최종 원고 승인을 운영 승인으로 대신하지 않는다.
...
[제약]
별도 운영 승인 전 운영 쓰기 호출 횟수는 0회이며 작업공간 파일 변경도 0회다. `development-preview-supervisor.mjs ensure`의 동일 worktree 미리보기 lease 재사용·재기동은 개발·운영 API 호출이나 Git 쓰기가 아니며 허용되지만, 직접 `npm run dev`, 임의 PID 종료 또는 포트 LISTEN만으로 준비 완료 선언은 금지한다. 승인 대기는 `needs_input` Blocked를 정확히 한 번만 사용하며 `review` Kanban 상태를 사용하지 않는다. `needs_input` 승인 대기 중에는 개발 미리보기를 종료하지 않는다. 운영 승인 후에도 이미지 push와 운영 API POST를 재시도하지 않는다. 모든 운영 외부 쓰기는 저장소·브랜치 전역 lock을 보유한 `production_write_once.py` 안에서만 수행한다. 최종 원고 승인, 개발 성공, 강제 스킬 지정 또는 일반 `/deploy` 요청만으로 운영 승인을 대신하지 않는다. 키, Authorization 헤더, 원시 인증 응답, Vercel CLI 사용, 현재 작업 폴더의 승인 전 변경과 승인 범위 밖 파일 반영을 금지한다.
...
- 운영 발행 terminal state에서 임시 서버 종료와 포트 해제 확인 결과
- 비밀값을 제외한 최종 상태

[합격 기준]
`draft: false` 개발 성공을 확인한 뒤 `scripts/development-preview-supervisor.mjs ensure`가 run별 detached lease에서 `/`·`/blog` HTTP 200 readiness를 보장하고 상세·목록·이미지 6개를 재검증한다. 죽은 lease는 동일 worktree·포트에서만 개발 POST 없이 재기동할 수 있다. 이후 작업공간을 변경하지 않고 운영 대상과 정확한 공개 범위를 읽기 전용으로 감사한다. 한 번의 `needs_input` Blocked 댓글에서 개발 미리보기 URL, 세 승인 의미와 slug별 `/deploy 승인` 복사 문구를 함께 제공하고, 이 대기 동안 미리보기를 유지한다. 대표가 개발 화면을 검토하고 번호형 한 줄 승인문을 새 댓글로 남긴 뒤에만 승인된 manifest를 deploy한다. 저장소·브랜치 전역 lock을 획득한 `production_write_once.py`가 최신 main 재확인부터 이미지 push·운영 POST 응답 확인까지 직렬화해야 한다. 이미지 push와 운영 POST는 각각 단일 시도이며 어떤 오류나 불명확한 결과에서도 재시도하지 않는다. 성공 후 lock을 해제하고 공개 글과 이미지 6장, 커밋 범위, lint와 build를 읽기 전용으로 검증한 뒤에만 supervisor의 `stop`으로 임시 서버를 종료하고 포트 해제를 확인한다.
...
- 개발 검증 카드가 `scripts/development-preview-supervisor.mjs ensure`로 `/`·`/blog` 순차 HTTP 200 readiness와 run별 lease를 확보한 뒤 `scripts/development-publish-once.mjs`로 해당 slug 한 건의 승인 원고·SHA-256·date `YYYY-MM-DD`·이미지 6장을 무쓰기 preflight하고, 같은 helper로 개발 API를 `draft: false`로 최대 1회 시작하며, 기존 시작 원장이 있으면 재호출을 거부하는지 확인한다. 이어서 상세 페이지의 승인 콘텐츠·본문 이미지 5장, 블로그 목록의 해당 글 링크·승인 썸네일 1장, 전체 이미지 파일 6장의 HTTP 200을 검증한 뒤 Blocked가 아닌 `done`으로 완료하되 worker 종료와 무관하게 lease는 운영 발행 카드 재검증과 운영 승인 대기가 끝날 때까지 유지하는지 확인한다.
- 운영 발행 카드가 먼저 supervisor `ensure`로 lease를 재사용하거나 개발 POST 없는 동일 worktree 재기동 뒤 개발 미리보기를 확인하고 대상·범위를 작업공간 무변경으로 감사하며, Blocked 댓글에 미리보기 URL·세 승인 항목·slug별 `/deploy 승인` 복사 문구를 남기고 번호형 별도 운영 승인을 받은 뒤 저장소·브랜치 전역 lock의 `production_write_once.py`로만 deploy하는지 확인한다. 운영 발행 terminal state에서만 supervisor `stop`으로 임시 서버를 종료하고 포트 해제를 확인하는지 점검한다.
